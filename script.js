...
submitBtn.addEventListener("click", (e) => {
  ...
  let read;
  let readInq = document.querySelectorAll('[name="read"]').forEach((readIt) => {
    if (readIt.checked) {
      read = readIt.value;
    }
  });
  let suck;
  let suckInq = document.querySelectorAll('[name="suck"]').forEach((suckIt) => {
    if (suckIt.checked) {
      suck = suckIt.value;
    }
  });
  ...
  if (
    ...
    let warning = document.querySelector(".warning").classList.add("invisible");
  }
...
function formReset() {
  document.querySelectorAll("form [type=text]").forEach((input) => {
    input.value = "";
  });

  document.querySelectorAll("form input:checked").forEach((checked) => {
    checked.checked = false;
  });

  document.querySelectorAll("#not-read, #unsure").forEach((button) => {
    button.checked = true;
  });
}
